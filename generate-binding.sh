...
rm -fR ./src/SlateDb-redist/runtimes > /dev/null 2>&1

cargo build

SRC_DIR="./target/debug"
DEST_DIR="./src/SlateDb-redist/runtimes"
...
cd ./src/SlateDb-redist

dotnet build -c Debug
dotnet pack -c Debug --include-symbols --include-source -o ../../packages

cd ../../

# dotnet nuget remove source local > /dev/null 2>&1
# dotnet nuget add source ./packages -n local

cd src/SlateDb

dotnet add package -s ../../packages SlateDb.Redist --version 1.0.0
